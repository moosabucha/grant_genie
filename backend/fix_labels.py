...
DOMAIN_KEYWORDS = {
    ...
    "Climate & Environment": [
        ...
        "emissions", "global warming", "flood", "ocean", "deforestation", "pollution"
    ],
    "Biomedical & Health": [
...
for researcher in researchers_list:
    ...
    for grant in same_domain:
        ...
        label      = "Good Fit" if overlap >= 0.33 else (
                     "Average Fit" if overlap >= 0.16 else "Bad Fit")
    ...
    for grant in diff_domain:
        if (len(good_pairs) >= TARGET_EACH and
            len(average_pairs) >= TARGET_EACH and
            len(bad_pairs) >= TARGET_EACH):
            break
        grant_text = f"{grant['title']} {grant['abstract']}"
        overlap    = compute_overlap(r_keywords, grant_text)
        label      = "Good Fit" if overlap >= 0.33 else (
