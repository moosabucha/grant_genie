...
def match_grants():
    ...
    best_algo, all_scores = select_best_algorithm(tfidf_results, rfuzz_results, hybrid_results)
    if best_algo == 'tfidf':
    ...
    return render_template('results.html', top_grants=top_grants, alt_pool=alt_pool, best_algo=best_algo, all_scores=all_scores, user=current_user)
    ...
    if os.path.exists(csv_path):
        ...
        for idx, row in df.iterrows():
            grants.append({
                ...
                'deadline'   : 'Open' if str(row.get('end_date', '')) in ['nan', '', 'None'] else str(row.get('end_date', 'Open')),
