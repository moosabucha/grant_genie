...
class RapidFuzzMatcher:
    def match(self, profile_text, grant_calls):
        ...
        for grant in grant_calls:
            ...
            score = round(fuzz.token_set_ratio(profile_text, grant_text) / 100 * 100, 1)
            results.append({**grant, 'score': score, 'algorithm': 'RapidFuzz', 'matched_features': self._get_matched_features(profile_text, grant, score)})
        return results
    def _get_matched_features(self, profile_text, grant, score):
        keywords = ['Artificial Intelligence', 'Machine Learning', 'Healthcare', 'NLP', 'Sustainability', 'Early Career', 'Postgraduate']
        matched = []
        for kw in keywords:
