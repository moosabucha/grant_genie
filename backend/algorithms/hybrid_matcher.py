...
class HybridMatcher:
    ...
    def match(self, profile_text, grant_calls):
        ...
        for grant in grant_calls:
            ...
            hybrid_score = round((t_score * self.tfidf_weight) + (r_score * self.rfuzz_weight), 1)
            tfidf_features = next((r['matched_features'] for r in tfidf_results if r['id'] == grant['id']), [])
            results.append({**grant, 'score': hybrid_score, 'algorithm': 'Hybrid (TF-IDF + RapidFuzz)', 'matched_features': tfidf_features, 'tfidf_score': t_score, 'rfuzz_score': r_score})
        return results
