...
class TFIDFMatcher:
        ...
        for i, grant in enumerate(grant_calls):
            ...
            results.append({
                **grant,
                'score': score,
                'algorithm': 'TF-IDF',
                'matched_features': self._get_matched_features(profile_text, grant, score)
            })
    ...
    def _get_matched_features(self, profile_text, grant, score):
        ...
        keywords = ['artificial intelligence', 'machine learning', 'healthcare',
                    'nlp', 'sustainability', 'early career', 'postgraduate', 'education']
        matched = []
        for kw in keywords:
